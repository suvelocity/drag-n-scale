...
describe("dragging box", () => {
  test("dragging", async () => {
    ...
    const { newTop, newLeft } = await page.evaluate((header) => {
      ...
      return {
        newTop: top + header.clientHeight / 2,
        newLeft: left + header.clientWidth / 2,
      };
    }, header);
  ...
  test("can't go past the borders", async () => {
    ...
    const { playTop, playLeft, playBottom, playRight } = await page.evaluate(
      (playground) => {
    ...
    const { y, x, headerWidth, headerHeight } = await page.evaluate(
      (header) => {
    ...
    const {
      onBottomRightCornerTop,
      onBottomRightCornerLeft,
    ...
    await page.mouse.move(
      onBottomRightCornerLeft + headerWidth / 2,
      onBottomRightCornerTop + headerHeight / 2
    );
    await page.mouse.down();
    ...
    const { onTopLeftCornerTop, onTopLeftCornerLeft } = await page.evaluate(
      (box) => {
        const { top, left } = box.getBoundingClientRect();
